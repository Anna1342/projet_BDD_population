...
-- 3.1 Liste des populations en 2020 par département avec leurs noms.
...
-- 3.2 Liste des populations en 2020 par région avec leurs noms.
...
-- 4.1 Population de Paris au total
...
-- 4.2 Population de Paris par arrondissement
...
-- Quel est le problème? la population est en double comptage car chaque arrondissement est compté comme une commune distincte avec un codGeo spécifique,
 -- Paris à la fois comme entité globale et via ses arrondissements=>incohérence dans les données à vérifier pour tout
...
SELECT d.nomDepart, (SUM(p.population) / c.superficie) AS densite_population
FROM Departement d 
...
-- est supérieur à la superficie moyenne et la population inférieur à la population moyenne
SELECT c.nomCommune, c.superficie, p.population
FROM Commune c 
	JOIN Population p ON c.codeCommune = p.nomCommune
WHERE c.superficie > (SELECT AVG(superficie) 
					FROM Commune)
AND p.population < (SELECT AVG(population) 
				  FROM Population);
